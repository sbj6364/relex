...
export type TodayFormState = { weeklyRemaining: string; clockIn: string; todayClockOut: string; fridayClockIn: string; remainingDayPlans: Record<string, PlannedWorkdayForm>; useDefaultBreak: boolean; customBreaks: TimeRange[] };
...
export function TodayCalculatorForm({ value, onChange, planningWeekdays, getDefaultTodayClockOut }: { value: TodayFormState; onChange: (value: TodayFormState) => void; planningWeekdays: number[]; getDefaultTodayClockOut: (clockIn: string) => string }) {
  ...
  return <div className="grid gap-4"><DurationInput label="Flex 상 이번 주 남은 근무시간" value={value.weeklyRemaining} onChange={(weeklyRemaining) => onChange({ ...value, weeklyRemaining })} hint="예: 07:40 또는 740 → 7시간 40분" /><div className="grid grid-cols-2 gap-3"><TimeInput label="오늘 출근" value={value.clockIn} onChange={(clockIn) => onChange({ ...value, clockIn, todayClockOut: getDefaultTodayClockOut(clockIn) })} /><TimeInput label="오늘 퇴근" value={value.todayClockOut} onChange={(todayClockOut) => onChange({ ...value, todayClockOut })} /></div>{planningWeekdays.length > 0 && <div className="grid gap-3 rounded-3xl bg-slate-50 p-4"><div><p className="text-sm font-black text-slate-700">금요일 전 근무 계획</p><p className="mt-1 text-xs font-medium leading-5 text-slate-500">내일부터 목요일까지 표시돼요. 기본값은 09:00-18:00이며, 수정하면 금요일 예상 퇴근 시간이 바로 다시 계산돼요.</p></div>{planningWeekdays.map((weekday) => { const plan = value.remainingDayPlans[weekday] ?? defaultPlan; return <div key={weekday} className="grid grid-cols-[1.5rem_minmax(0,1fr)_minmax(0,1fr)] items-end gap-2"><div className="pb-3 text-sm font-black text-slate-600">{weekdayLabels[weekday]}</div><TimeInput label="출근" value={plan.clockIn} onChange={(clockIn) => updatePlan(weekday, { ...plan, clockIn })} /><TimeInput label="퇴근" value={plan.clockOut} onChange={(clockOut) => updatePlan(weekday, { ...plan, clockOut })} /></div>; })}</div>}<TimeInput label="금요일 예상 출근 시간" value={value.fridayClockIn} onChange={(fridayClockIn) => onChange({ ...value, fridayClockIn })} /><Toggle label="기본 점심시간 적용" checked={value.useDefaultBreak} onChange={(useDefaultBreak) => onChange({ ...value, useDefaultBreak })} description="기본값 12:00-13:00" /><BreakEditor breaks={value.customBreaks} onChange={(customBreaks) => onChange({ ...value, customBreaks })} /></div>;
}
