...
const initialForm: TodayFormState = { weeklyRemaining: '08:00', clockIn: '09:00', currentTime: now(), fridayClockIn: '09:00', remainingDayPlans: defaultRemainingDayPlans, useCurrentTime: true, useDefaultBreak: true, customBreaks: [] };

function getInitialForm() {
  const saved = loadFromStorage(storageKey, initialForm);
  return { ...initialForm, ...saved, remainingDayPlans: { ...defaultRemainingDayPlans, ...saved.remainingDayPlans } };
}
...
function getPlanningWeekdays(todayWeekday: number) {
  ...
  return Array.from({ length: friday - todayWeekday }, (_, index) => todayWeekday + index);
}
...
export function TodayPage({ rules }: { rules: WorkRules }) {
  ...
  const result = useMemo(() => {
    try {
      ...
      const plannedWorkdaysBeforeFriday = Object.entries(form.remainingDayPlans).map(([weekday, plan]) => ({ weekday: Number(weekday), clockIn: parseTime(plan.clockIn), clockOut: parseTime(plan.clockOut) }));
  ...
  return <div className="grid gap-5 lg:grid-cols-[1fr_1.05fr]"><TodayResultCard {...result} /><Card><div className="mb-5"><h2 className="text-xl font-black">금요일 퇴근 계산</h2><p className="mt-1 text-sm text-slate-500">Flex 상 주간 잔여시간을 입력하면 금요일 전 근무 계획을 반영해 금요일 예상 퇴근 시간을 알려드려요. 남은 날들은 기본 09:00-18:00으로 시작하고 바로 수정할 수 있어요.</p></div><TodayCalculatorForm value={form} onChange={setForm} planningWeekdays={planningWeekdays} /></Card><Card className="lg:col-span-2"><h2 className="text-lg font-black">기본 규칙</h2><p className="mt-2 text-sm text-slate-500">근무 가능 시간 {formatTime(rules.workWindow.start)}-{formatTime(rules.workWindow.end)}, 코어타임 {formatTime(rules.coreTime.start)}-{formatTime(rules.coreTime.end)}, 기본 휴게 {formatTime(rules.defaultBreak.start)}-{formatTime(rules.defaultBreak.end)} 기준으로 계산합니다. 월~목 계산은 화면의 금요일 전 근무 계획을 기준으로 합니다.</p>{rules === defaultWorkRules && null}</Card></div>;
}
